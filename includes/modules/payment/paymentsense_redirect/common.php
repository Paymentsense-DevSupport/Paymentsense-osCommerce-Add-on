...
*/
	...
	function request_status($code, $version) {
		if ($_GET['set'] == "payment") {
			$headers = array('SOAPAction:http://www.csovc.co.uk/VersionCheck','Content-Type: text/xml; charset = utf-8','Connection: close');
			$SCFID = $code;
			$SCV = trim(implode('', file(DIR_FS_CATALOG . 'includes/version.php')));						
			$MV = $version;
			$xml = '<?xml version="1.0" encoding="utf-8"?><soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"><soap:Header><authentication xmlns="http://www.csovc.co.uk"><User>bc7ef304a43cf6bf8903cd3224f3997b</User><Password>745463d8a9156fe9eecec0d5b6501b15</Password></authentication></soap:Header><soap:Body><VersionCheck xmlns="http://www.csovc.co.uk"><SCID>26</SCID><SCFID>'. $SCFID .'</SCFID><SCV>'. $SCV .'</SCV><MV>'. $MV .'</MV><SAD>'. $_SERVER['HTTP_HOST'] .'</SAD></VersionCheck></soap:Body></soap:Envelope>';
			$url = 'http://www.csovc.co.uk/versioncheck.asmx';		
			...
			if($err == 0) {
				if( preg_match('#<UpdateAvailable>([a-zA-Z0-9]+)</UpdateAvailable>#iU', $ret, $U) ) {
					$UA = $U[1];
					if($UA == "True") {
						preg_match('#<NewVersion>([a-zA-Z0-9.]+)</NewVersion>#iU', $ret, $V);
						$NV = $V[1];
						preg_match('#<UpdateURL>([*a-z*A-Z*0-9*:*/*.*\-\*_]+)</UpdateURL>#iU', $ret, $URI);
						$UURI = $URI[1];
						$_SESSION['updates'] = ' - Version ' . $NV . ' Available';				
					}
				}
			}
		}
